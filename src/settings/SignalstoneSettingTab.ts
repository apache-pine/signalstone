...
export class SignalstoneSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(this.containerEl).setName('Webex access token').setDesc('Select a token from Obsidian secret storage. Personal developer tokens expire after about 12 hours and are intended for development and testing.').then((setting) => {
		...
		new Setting(this.containerEl).setName('Debug logging').setDesc('Logs realtime connection and message-delivery activity to the developer console, to help diagnose problems. Never logs your token or message content. Off by default.').addToggle((toggle) => toggle.setValue(this.plugin.settings.debugLogging).onChange(async (value) => { this.plugin.settings.debugLogging = value; await this.plugin.saveSettings(); this.plugin.applyDebugLogging(); }));
